...
export class LoggingLogic extends LogicBase {
    ...
    protected onMessage(
        ...
        const dict =
            message.Dictionary === undefined
                ? ""
                : `; dict: ${JSON.stringify(message.Dictionary)}`;
        if (message.Type === "Hidden") {
            logger.debug(
                `Message ${message.Type} from ${sender.Name} (${sender.MemberNumber}): ` +
     ...
     */
    protected onCharacterMetadataChanged(
        connection: API_Connector,
        character: API_Character,
        joining: boolean,
    ): void {
        if (joining) return;
        logger.info(
            `Character ${character.Name} (${character.MemberNumber}) metadata changed`,
        );
    }
    ...
    protected onCharacterEvent(
        ...
        switch (event.name) {
            ...
            case "PoseChanged":
                logger.info(
                    `PoseChanged for ${event.character.Name}: ${event.character.Pose.map((P) => P.Name)}`,
                );
                break;
            case "SafewordUsed":
                logger.alert(
                    `Character ${event.character.Name} used swafeword! (full release: ${event.release})`,
                );
                break;
            default:
                // @ ts-expect-error: We shouldn't reach this
                logger.info(
                    `Unknown character event ${event.name} from ${event.character.Name}`,
                );
                break;
        }
    }
