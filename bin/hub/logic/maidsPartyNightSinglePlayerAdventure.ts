...
import {
    ...
    JMod_allowApplyAppearanceBundle,
    JMod_importAppearanceBundle,
    JMod_exportAppearanceBundle,
    JMod_applyAppearanceBundle,
} from "bc-bot";
...
const listOfUsedItemsInThisScene: (
    | [AssetGroupName, string]
    | [AssetGroupName, string, string]
)[] = [
    ["ItemHandheld", "Paddle"],
    ["ItemMouth", "DusterGag"],
...
export class MaidsPartyNightSinglePlayerAdventure extends LoggingLogic {
    ...
    introductionProgress: IntroductionProgress =
        IntroductionProgress.meetingTheHeadMaid;
	...
	*/
    ...
    resetTimeoutTimer(reason: "disconnect" | "afk" | "afkwarn") {
        ...
        if (this.player !== null) {
            ...
            this.timer = setTimeout(() => {
                void this.cleanUpSinceActivePlayerWasLostForThisReason(reason);
            }, 300_000);
        }
    }
    ...
    playerGreeting(character: API_Character, everything: boolean) {
        if (everything) {
            ...
            character.Tell(
                "Emote",
                "*by Claudia and Jomshir. It is a demo of a choice-based single player experience for submissives, with the goal " +
        ...
        character.Tell(
            "Emote",
            "*You can (reset the scene fully) at any point in time with '*reset the scene fully' " +
        ...
        if (!character.CanTalk()) {
            character.Tell(
                "Whisper",
                `Because you are gagged, you may not be able to use whispers depending on your settings. Therefore, only right now, ` +
    ...
    async playerCheck(character: API_Character): Promise<boolean> {
        ...
        if (!character.ProtectionAllowInteract()) {
            character.Tell(
                "Chat",
                `Warning: The bot currently cannot interact with you ` +
            ...
            logger.info(
                `Player check for ${character}: Protection: Version=${character.OnlineSharedSettings.GameVersion}, Admin=${this.conn.Player.IsRoomAdmin()}`,
            );
            hadWarnings = true;
        } else if (!allow) {
            character.Tell(
                "Chat",
                `Warning: The bot currently cannot interact with you because of your permission settings. Please change them or white list the bot.`,
            );
            //logger.info(`Player check for ${character}: Permission level: ` + BC_PermissionLevel[character.ItemPermission]);
            hadWarnings = true;
        }
        const itemsCannotRemove = character.Appearance.Appearance.filter(
            (A) => listOfUsedItemGroups.includes(A.Group) && !A.AllowRemove(),
        );
        if (itemsCannotRemove.length > 0) {
            character.Tell(
                "Chat",
                `Warning: Scenario will conflict with following restraints you have on you:\n` +
                    itemsCannotRemove.map((A) => A.Name).join(", ") +
                    `\nScenario cannot remove these because of locks or other limiting factors`,
            );
            logger.info(
                `Player check for ${character}: Unremovable items: ` +
                    itemsCannotRemove.map((A) => A.Name).join(", "),
            );
            hadWarnings = true;
        }
        const itemsToRemove = character.Appearance.Appearance.filter(
            (A) => listOfUsedItemGroups.includes(A.Group) && A.AllowRemove(),
        );
        if (itemsToRemove.length !== 0) {
            character.Tell(
                "Chat",
                `Warning: Starting the scenario will remove the following restraints you have on you:\n` +
                    itemsToRemove.map((A) => A.Name).join(", "),
            );
            logger.info(
                `Player check for ${character}: Items to remove: ` +
                    itemsToRemove.map((A) => A.Name).join(", "),
            );
            hadWarnings = true;
        }
        const itemsCannotUse = listOfUsedItemsInThisScene.filter(
            (item) =>
                !character.IsItemPermissionAccessible(
                    AssetGet(item[0], item[1]),
                    item[2],
                ),
        );
        if (itemsCannotUse.length > 0) {
            character.Tell(
                "Chat",
                `Warning: The scenario uses following items, but you have them blocked or limited:\n` +
                    itemsCannotUse.map((A) => A.join(":")).join(", "),
            );
            logger.info(
                `Player check for ${character}: Blocked items: ` +
                    itemsCannotUse.map((A) => A.join(":")).join(", "),
            );
            hadWarnings = true;
        }
        const outfitsCannotUse = Object.entries(listOfAllPlayerOutfits)
            .filter(([k, v]) => {
                return !JMod_allowApplyAppearanceBundle(
                    character,
                    JMod_importAppearanceBundle(v),
                    {
                        appearance: false,
                        bodyCosplay: false,
                        clothing: true,
        ...
        if (allow && outfitsCannotUse.length > 0) {
            character.Tell(
                "Chat",
                `Warning: Following outfits used by scenario cannot be used on you,` +
                    `because you have some clothing blocked or limited:\n` +
                    outfitsCannotUse.join(", "),
            );
            logger.info(
                `Player check for ${character}: Blocked outfits: ` +
                    outfitsCannotUse.join(", "),
            );
            hadWarnings = true;
        }
        if (hadWarnings) {
            character.Tell(
                "Whisper",
                `These warnings won't prevent you from playing, but may cause your experience to be degraded.\n` +
    ...
    playerGenericEnd(character: API_Character, toBeContinued: boolean) {
        ...
        character.Tell(
            "Whisper",
            `${toBeContinued ? "END OF THE DEMO: You have reached the current" : "END: You have reached the"} ` +
     ...
     */
    protected async onCharacterEntered(
		...
		*/
        ...
        if (
            ...
            if (SUPERUSERS.includes(character.MemberNumber)) {
                this.conn.SendMessage(
                    "Chat",
                    `Welcome back, Mistress ${character.Name}!`,
                );
     ...
     */
    protected async onCharacterLeft(
        connection: API_Connector,
        character: API_Character,
        intentional: boolean,
    ): Promise<void> {
		...
		*/
        ...
        if (this.player === character && (intentional || !this.started)) {
            ...
            this.conn.SendMessage(
                "Emote",
                "*NOTE: Since the player disconnected, we will now wait 5 minutes before the room will be reset.",
            );
    ...
    protected async onMessage(
        ...
        if (
            sender === this.player &&
            ["Chat", "Whisper"].includes(message.Type) &&
            /^.?help($|\W)/i.test(message.Content)
        ) {
        ...
        if (sender === this.player && message.Type === "Emote") {
            ...
            if (
                msg.includes("reset") &&
                (msg.includes("scene fully") || msg.includes("room fully"))
            ) {
                await this.cleanUpActionsAsActivePlayerTriggeredRoomReset(
                    sender,
                );
                ...
                if (msg.includes("leave")) {
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*The image of the bot disappears as the room starts to power up, forming a club corridor around ${sender.Name}, ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} is one of the many maids the bondage club employs, just entering the ` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `${sender.Name}: Good evening, everyone!`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*In that very moment, Head Maid Karina turns around, immediately approaching and greeting the new arrival.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: Hello ${sender.Name}, I urgently need you to get ready for a special ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*She smirks quite a bit at ${sender.Name} as she says that.`,
                    );
                    sender.Tell(
                        "Whisper",
                        `Possible reactions:` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `${sender.Name}: Of course, Miss Karina, when does it start?`,
                    );
                    if (this.ifUndressed(sender)) {
                        this.toldToStripFully = true;
                        this.conn.SendMessage(
                            "Chat",
                            `Head Maid Karina: Good. As soon as you are ready.\n...and seeing as you are already naked, ` +
                        ...
                        this.conn.SendMessage(
                            "Chat",
                            `Head Maid Karina: Good. You will be a pleasing enough sight for tonight's special guests. Let's get you ` +
                                `ready then. Hold still, dear, while we work on you.`,
                        );
                        this.conn.SendMessage(
                            "Emote",
                            `*${sender.Name} (nods slowly), witnessing a few of the other maids smirking and seemingly quite looking ` +
                                `forward to what is bound to happen next.`,
                        );
                        this.introductionProgress =
                            IntroductionProgress.acceptingTheAssignment;
                        return;
                    }
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: Good. As soon as you are ready. Now be a dear and strip fully, I need to check ` +
                    ...
                    this.introductionProgress =
                        IntroductionProgress.acceptingTheAssignmentStrip;
                    return;
                } else if (msg.includes("sorry")) {
                    this.printChatSeparator();
                    this.conn.SendMessage(
                        "Chat",
                        `${sender.Name}: Oh, uuhm, I am very sorry Miss, but I really don't feel like being exposed to a public audience tonight... `,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*The head maid looks quite disappointed and orders ${sender.Name} to immediately (change) into her uniform.`,
                    );
                    this.introductionProgress =
                        IntroductionProgress.refusingTheAssignment;
                    return;
                } else if (!this.hintShown) {
                    sender.Tell(
                        "Whisper",
                        `HINT: To proceed, type an emote into the chat using the word in round brackets. ` +
                ...
                if (msg.includes("nods") && this.ifUndressed(sender)) {
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*Karina looks pleased at ${sender.Name}, then turns around, snapping her fingers. One of the ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*Trixie looks encouragingly at ${sender.Name}, immediately getting to work, giving her friendly instructions ` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Trixie: ...aaand all done! Please wait here while I get the restraints.`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*The maid walks off towards a cabinet while ${sender.Name} looks with wide eyes down at the new oufit that is ` +
                    ...
                    (msg.includes("say") && msg.includes("nothing")) ||
                    ...
                    if (msg.includes("promise")) {
                        this.conn.SendMessage(
                            "Chat",
                            `${sender.Name}: I am s-sorry, Miss Karina, please don't punish me! I will work hard tonight, I promise!`,
                        );
                        this.conn.SendMessage(
                            "Chat",
                            `Head Maid Karina: Hmmm... Fine, you will get another chance to impress me! Trixie, continue please.`,
                        );
                    }
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} silently watches as Trixie comes back with an assortment of restraints and items. First she grabs ` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Trixie: Arms behind your back, please~`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} clearly accepted her role for tonight, ` +
                    ...
                    sender.Appearance.AddItem(
                        AssetGet("ClothLower", "PencilSkirt"),
                    )?.SetColor("#460000");
                    this.introductionProgress =
                        IntroductionProgress.gettingReady;
                    return;
                } else if (msg.includes("no wait")) {
                    this.printChatSeparator();
                    this.conn.SendMessage(
                        "Chat",
                        `${sender.Name}: Eeeh?~ No, no, wait! I don't feel like going to it all tied up agaaain!`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*The head maid comes back around, looking very displeased at ${sender.Name}, grabbing her chin strictly and ` +
                            `staring deeply into her eyes.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: I see~~ Instead you feel like wasting our time, hmm? That's alright then, ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} squirms a bit, but knowing that she overdid it and Karina would not let her get away with ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*Karina nods content and snaps her fingers, prompting Trixie to lead ${sender.Name} to a nearby sofa ` +
                    ...
                    const item = this.conn.Player.Appearance.AddItem(
                        AssetGet("ItemHandheld", "Paddle"),
                    );

                    this.conn.SendMessage(
                        "Emote",
                        `*The head maid smirks, stepping behind ${sender.Name} with the paddle. She follows up by slowly lifting ` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: You will count the strikes out loud! If you make a mistake, ` +
                            `we will start over~`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*She clearly takes her time with the first strike, letting the tension rise.`,
                    );
                    await wait(26000);
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*After about half a minute, Karina swings down the paddle, giving ${sender.Name}'s butt cheeks ` +
                    ...
                    this.introductionProgress =
                        IntroductionProgress.recievingPunishment;
                    return;
                } else {
                ...
                if (msg.includes("yes")) {
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `${sender.Name}: Y-Yes, Miss Karina`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*Karina smiles a bit, but clearly expects ${sender.Name} to promptly strip, a hint of impatience in her eyes.`,
                    );
                    return;
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*The head maid looks quite displeased at ${sender.Name}'s sudden refusal and coldly orders her to immediately (change) into her uniform.`,
                    );
                    this.introductionProgress =
                        IntroductionProgress.refusingTheAssignment;
                    return;
                ...
                if (msg.includes("change")) {
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} quickly moves to the dressing room and changes into the standard maid uniform, before ` +
                            `rushing back to the head maid, who looks at ${sender.Name} in a somewhat mocking way, holding a duster gag in her hands.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: Since you refused the assignment, let's make you otherwise useful for tonight. Open wide, dear!`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} knows that Karina won't accept yet another refusal without severe consequences ` +
					...
					*/
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*With a teasing clap on ${sender.Name}'s butt cheeks, the grinning head maid sends her down the hallways ` +
                    ...
                    logger.alert(
                        `Player ${sender.Name} (${sender.MemberNumber}) reached ending: Storage cleaning`,
                    );
                ...
                if (msg.includes("watch")) {
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*The maid carefully buckles the tray around ${sender.Name}'s hips, its strap going comfortably around her neck. ` +
                            `Following that, Trixie bring an assortment of various drinks for the party and adds them carefully onto the tray.`,
                    );
                    tray = sender.Appearance.AddItem(
                        AssetGet("ItemMisc", "ServingTray"),
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Trixie: I think you should be fine balancing it like this. Now there is just the fun part missing~`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} watches with widening eyes as the upbeat maid brings a box ` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Trixie: It is best to be prepared! Who knows what tonight's guests might enjoy~`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*The maid smiles sweetly and dreamy at ${sender.Name}, who can do nothing much than mumble a bit into her gag, ` +
                            `watching the tray getting quite full and weighty. Eventually, Trixie turns around and waves happily towards the head maid.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Trixie: Miss Kariiinaa~ My lovely maid sister is all dolled up and ready~`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*As ${sender.Name} (turns) her head, she sees the head maid walking up to them again, grinning a bit ` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: Well done, Trixie. She indeed looks appropriately equipped for tonight's party.`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*After saying that, she turns her gaze to ${sender.Name}, smiling somewhat warmly.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: Alright, ${sender.Name}. You know what is expected of you. Entertain tonight's club ` +
                ...
                if (msg.includes(paddleHitNumbers[this.paddleHitCount])) {
                    if (this.paddleHitCount === 9) {
                        ...
                        this.conn.SendMessage(
                            "Chat",
                            `Head Maid Karina: There you go. You took those admirably. I am sure you learned your lesson in good behavior now.`,
                        );
                        this.conn.SendMessage(
                            "Emote",
                            `*Karina leans down and gently brushes over ${sender.Name}'s head, sending her together with Trixie ` +
                                `to the maid rest room for after care and an early end of today's work.`,
                        );
                        this.conn2.ChatRoomLeave();
                        this.changeBotAppearanceTo("trixie", this.conn);
                        logger.alert(
                            `Player ${sender.Name} (${sender.MemberNumber}) reached ending: Head Maid punishment`,
                        );
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: Ah~ Ah~ That was wrong. Seems ` +
                            `we will start at (one) again~`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*She swings the paddle again, giving ${sender.Name}'s butt cheeks another hit.`,
                    );
                    await wait(2500);
                    if (
                        this.storyProgress !== StoryProgress.introduction ||
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: Be serious and speak loud and clear!`,
                    );
                }
            } else if (
                this.storyProgress === StoryProgress.theParty &&
                ...
                if (msg.includes("go") && msg.includes("to hall")) {
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} leaves the maid quarters behind, making her way in slow hobbled steps towards hall 3. ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} slowly approaches the entrance, trying to not spill any of the drinks. ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*Not wanting to be tangled up in this, ${sender.Name} quickly continues on to the entrance and (enters).`,
                    );
                    return;
                ...
                if (msg.includes("stunning lady")) {
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} approaches the gorgeous lady curiously, eyeing her from head to toe, wondering ` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: My, my, what a cute maid! Are you here to let me play with you?`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*She smirks quite a bit as she says that, her gaze playfully teasing, but leaving no doubt ` +
                    ...
                    this.teasingLadyProgress =
                        TeasingLadyProgress.meetingForTheFirstTime;
                    return;
                } else {
                    /* the chat emote/action was nothing recognized, therefore do nothing. */
                    return;
                ...
                if (msg.includes("raise")) {
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*The lady grins somewhat predatory and claps her hands together with mock enthusiasm.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: Wonderful! You even brought the necessary toys for that with you.`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} gets quite big eyes from those words, quickly shaking her head, which seems to widen her grin.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: Reeaaally? Why would you tease me with all those toys, if you don't want me to use them?~`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*The lady's hand raises towards the tray and slowly and playfully moves along the selection ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*After the lady's fingers almost completed their tour of the tray's kinky items, she stops ` +
                            `above a pair of nipple clamps.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: How about those cute clamps? Wouldn't these be just the perfect decoration for ` +
                            `a sweet little maid, who is showing off her nipples so cheekily?`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*She grins somewhat sadistically while her ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} slowly nods her head, apparently agreeing with this idea.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: Oh my!~ Is that so? You want me to clamp them onto your breasts, so you can lewdly show ` +
                            `them off to all the other party guests?`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*The lady smirks happily, clearly having ${sender.Name} right where she wants her.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: But who says that naughty little subbies get what they want?~ `,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*Her smirk widens a lot as she takes a glass of wine from the tray and steps back, ` +
                            `looking quite teasing.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: Thanks for the drink. Now move along dear! I am sure you will find ` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `${sender.Name}: Ne...faeahe, nee ehe kaamfh!`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: Aww~ What a bad little maid you are, telling me "No" when you should be telling me "Yes, Miss"~`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*She elegantly walks in a half circle around ${sender.Name} while her fingers trail teasingly ` +
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: This is not about you, dear. It's about serving guests and making them happy!~ ` +
                            `There is no need for your cute mumbling when you don't understand that, sweetie.`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*She giggles teasingly and takes a glass of wine from the tray after coming back around to ${sender.Name}'s front.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Lady: Thanks for the drink. Now move along and remember this lesson. Good things will only come to good girls~`,
                    );
                    await wait(16000);
                    const ballGag = sender.Appearance.AddItem(
                        AssetGet("ItemMouth", "BallGag"),
                    );
                ...
                this.storyProgress === StoryProgress.theParty &&
                this.teasingLadyProgress ===
                    TeasingLadyProgress.meetingForTheSecondTime
            ) {
                if (msg.includes("hobble")) {
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*${sender.Name} approaches the gorgeous lady again. However, she merely smirks mockingly at the ` +
                    ...
                    logger.alert(
                        `Player ${sender.Name} (${sender.MemberNumber}) reached ending: Club lady - end of the demo-`,
                    );
                ...
                logger.warning(
                    `Part of the code was reached that should not be able to be reached:` +
    ...
    private commonEndOfFirstEncounterWithTheTeasingLady(sender: API_Character) {
        this.conn.SendMessage(
            "Emote",
            `*The woman clearly seems to be done with the maid, her beautiful grinning face slowly turning away from ` +
    ...
    private async handleCommand(
        ...
        if (cmd.startsWith("!")) {
            ...
            let msg =
                `MESSAGE from ${sender.Name} (${sender.MemberNumber}):\n` +
                message.Content;
            logger.alert(msg);
            msg +=
                `\n[debug data]: ` +
                JSON.stringify({
                    ...
                    charPos: Array.from(this.charPos.entries()).map((c) => [
                        c[0].MemberNumber,
                        c[1],
                    ]),
            ...
            cmd.startsWith("end simulation") ||
            cmd.startsWith("stop simulation")
        ) {
            ...
            if (admins.length > 0) {
                msg +=
                    `\nAlternatively, you can also speak directly into the chat, as the following characters are also humans: ` +
                    admins.map((C) => C.Name).join(", ");
            }
    ...
    async pointsOfInterestAtEntranceForMaid(character: API_Character) {
        ...
        this.conn.SendMessage(
            "Emote",
            `*Hall 3 is one of the biggest in the club and consists of several sections. The entrance area ` +
        ...
        this.conn.SendMessage(
            "Emote",
            `*When ${character.Name} looks around, several things stick out. ` +
     ...
     */
    protected async onCharacterEvent(
        connection: API_Connector,
        ...
        if (event.name === "ItemRemove" && event.character === this.player) {
            if (
                this.ifUndressed(event.character) &&
                this.storyProgress === StoryProgress.introduction &&
                this.introductionProgress ===
                    IntroductionProgress.acceptingTheAssignmentStrip
            ) {
                ...
                this.conn.SendMessage(
                    "Emote",
                    `*The head maid watches ${event.character.Name} stripping with a somewhat pleased expression. She starts ` +
                ...
                this.conn.SendMessage(
                    "Emote",
                    `*${event.character.Name} (nods slowly), witnessing a few of the other maids smirking and seemingly quite looking ` +
                        `forward to what is bound to happen next.`,
                );
                this.introductionProgress =
                    IntroductionProgress.acceptingTheAssignment;
                ...
                this.introductionProgress ===
                    IntroductionProgress.acceptingTheAssignmentStrip &&
                !this.toldToStripFully
            ) {
                this.conn.SendMessage(
                    "Chat",
                    `Head Maid Karina: Strip fully, dear. Don't keep me waiting!`,
                );
            ...
            event.name === "PoseChanged" &&
            event.character === this.player
        ) {
            if (
                event.character.Pose.some((P) =>
                    ["BackCuffs", "BackElbowTouch", "BackBoxTie"].includes(
                        P.Name,
                    ),
                ) &&
                this.storyProgress === StoryProgress.introduction &&
                this.introductionProgress ===
                    IntroductionProgress.gettingReady &&
                event.character.Appearance.InventoryGet("ItemArms") === null
            ) {
                this.printChatSeparator();
                this.conn.SendMessage(
                    "Emote",
                    `*After ${event.character.Name} moves her hands behind her back, Trixie gently pushes them together, while slipping ` +
                ...
                this.conn.SendMessage(
                    "Chat",
                    `Trixie: Wonderful~~ Now... please part those sweet lips. I also have a nice gag for you~`,
                );
                this.conn.SendMessage(
                    "Emote",
                    `*The beaming maid looks at ${event.character.Name} with eager, sparkling eyes, waiting expectantly.`,
                );
                await wait(3500);
                const armbinder = event.character.Appearance.AddItem(
                    AssetGet("ItemArms", "LeatherArmbinder"),
                );
            ...
            if (
                ...
                if (
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*The maid smiles happily, watching ${event.character.Name} opening her mouth, and gently pushes in a red ball gag.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Trixie: Perfect! You look soo good like this. Now let me just get your serving tray.`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*Having the ball gag snugly buckled around ${event.character.Name}'s head she cannot do much more than mumble a few ` +
                            `incoherent words as she (watches) Trixie getting a large serving tray and bringing it back with her.`,
                    );
                    await wait(3000);
                    const ballGag = event.character.Appearance.AddItem(
                        AssetGet("ItemMouth", "BallGag"),
                    );
                    ballGag?.SetColor(["#FE3B3B", "Default"]);
                ...
                if (
                    ...
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: Good girl~`,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*The duster gag quickly finds its way into ${event.character.Name}'s mouth, tightly buckled and locked by the smirking ` +
                            `head maid.`,
                    );
                    this.conn.SendMessage(
                        "Chat",
                        `Head Maid Karina: Now (go and clean) the storage rooms. I expect it to be spotless!`,
                    );
                    await wait(3000);
                    // TODO: also add a Mistress lock (bot does not yet support locks)
                    event.character.Appearance.AddItem(
                        AssetGet("ItemMouth", "DusterGag"),
                    );
                    return;
                }
            } else {
            ...
            this.conn.SendMessage(
                "Emote",
                `*The room rapidly powers down as the emergency shutdown is triggered, leaving ${event.character.Name} in ` +
            ...
            event.character.Tell(
                "Whisper",
                `Dear ${event.character.Name}, here are the authors of this narrative. We are hoping you are fine, ` +
     ...
     */
    freePlayerInItemSlots(
        character: API_Character,
        itemSlots: AssetGroupName[],
    ) {
     ...
     */
    async cleanUpSinceActivePlayerWasLostForThisReason(
        reason: "left" | "disconnect" | "afk" | "afkwarn",
    ) {
        switch (reason) {
            case "left":
                this.conn.SendMessage(
                    "Emote",
                    "*NOTE: Since the player has left the room, the scene will be reset.",
                );
            ...
            case "disconnect":
                this.conn.SendMessage(
                    "Emote",
                    "*NOTE: Since the player disconnected and failed to return in time, the scene will be reset.",
                );
                await this.resetRoom(null);
                await this.makeRoomSizeOneMoreThanAdminsPresentAndUnlockRoom();
                break;
            ...
            case "afkwarn":
                this.conn.SendMessage(
                    "Emote",
                    `*WARNING: You have been afk for 5 minutes, ${this.player?.Name}. ` +
     ...
     * @param character the current player
     */
    async cleanUpActionsAsActivePlayerTriggeredRoomReset(
        character: API_Character,
    ) {
        this.printChatSeparator();
        this.conn.SendMessage(
            "Emote",
            `*The room powers down, leaving ${character.Name} in ` +
                `an empty room like when she entered, the calm face of the service bot greeting her.`,
        );
        await wait(2500);
        if (character != null) {
        ...
        if (
            ...
            logger.warning(
                `Failed to set ${character.Name}'s appearance to what they joined with!`,
            );
        }
        await wait(2500);
        this.playerGreeting(character, true);
    ...
    developerJumpsToLatestPart() {
        ...
        this.conn.SendMessage(
            "Emote",
            `*- Progress variables changed -\n Jump to the latest scene with (enter).`,
        );
        if (this.player !== null) {
            const ballGag = this.player.Appearance.AddItem(
                AssetGet("ItemMouth", "BallGag"),
            );
            ballGag?.SetColor(["#FE3B3B", "Default"]);
            ballGag?.Extended?.SetType("Shiny");
    ...
    changeBotAppearanceTo(npc: string, bot: API_Connector) {
        ...
        const newAppearance = JMod_importAppearanceBundle(
            listOfAllNPCsOutfits[npc],
        );
        if (!JMod_applyAppearanceBundle(bot.Player, newAppearance)) {
            logger.warning(
                `Failed to set ${bot.Player.Name}'s appearance to ${npc}!`,
            );
        }
    }
    ...
    resetBotExpressions(bot: API_Connector) {
        const allExpressionGroups: ExpressionGroupName[] = [
            "Blush",
            "Emoticon",
            "Eyebrows",
            "Eyes",
    ...
    changePlayerApperanceTo(playerOutfit: string) {
        if (!listOfAllPlayerOutfits[playerOutfit]) {
            logger.error(
                `Unable to find outfit ${playerOutfit} for the player ${this.player?.Name}`,
            );
            return;
        }
        const newAppearance = JMod_importAppearanceBundle(
            listOfAllPlayerOutfits[playerOutfit],
        );
        if (this.player !== null) {
            if (
                !JMod_applyAppearanceBundle(
                ...
                logger.warning(
                    `Failed to set ${this.player?.Name}'s appearance to ${playerOutfit}!`,
                );
            }
        } else {
