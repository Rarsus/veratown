...
type AdminCommandHandlerRaw = (
    connection: API_Connector,
    args: string,
    sender: API_Character,
) => void | Promise<any>;
type AdminCommandHandlerParsed = (
    connection: API_Connector,
    argv: string[],
    sender: API_Character,
) => void | Promise<any>;
type AdminCommandHandlerSu = (
    connection: API_Connector,
    argv: string[],
    sender: number,
...
export class AdministrationLogic extends LogicBase {
    private readonly a_settings: IAdminLogicSettings = {
        ...
        inactivityKickTimer: /*15 * 60 * 1000*/ null,
    ...
    private a_commands: Map<string, IAdminCommandRaw | IAdminCommandParsed> =
        new Map();
    ...
    private a_notBanComfirmation: WeakMap<API_Character, number> =
        new WeakMap();
    ...
    constructor(
        settings: Partial<IAdminLogicSettings>,
        ...
        this.registerCommand(
            "help",
            (connection, args, sender) => this.on_Help(sender),
        ...
        if (this.a_settings.kickVotingEnabled) {
            ...
            this.registerCommand(
                "kick",
                this.a_command_kick.bind(this, false),
                "Starts a vote to kick player",
            );
            this.registerCommand(
                "ban",
                this.a_command_kick.bind(this, true),
                null,
            );
        }
        ...
        if (this.a_settings.votingEnabled) {
            this.registerCommand(
                "yes",
                this.a_CharacterVote.bind(this, true),
                null,
            );
            this.registerCommand(
                "no",
                this.a_CharacterVote.bind(this, false),
                null,
            );
            this.registerCommand(
                "listvotes",
                this.a_command_listVotes.bind(this),
                "List currently ongoing votes",
            );
        }

        this.registerCommand(
            "feedback",
            this.a_command_feedback.bind(this),
            "Send a message as feedback to the authors",
        );

        const suRunner: AdminCommandHandlerParsed = (
            connection,
            argv,
            sender,
        ) =>
            this.a_command_su(
                ...
                argv,
                sender.MemberNumber,
                (response) => sender.Tell("Whisper", response),
            );
        ...
        this.registerSUCommand("say", (connection, argv, sender, respond) => {
            ...
            if (msg) {
                logger.info(
                    `${this.a_LogHeader(connection)} Admin chat from ${sender}: ${msg}`,
                );
        ...
        this.registerSUCommand("kick", (connection, argv, sender, respond) => {
            ...
            if (typeof target === "string") {
                ...
                connection.SendMessage(
                    "Emote",
                    `*${target} has been kicked by administrator.`,
                );
                return target.Kick();
            }
        });
        ...
        this.registerSUCommand("ban", (connection, argv, sender, respond) => {
            ...
            if (typeof target === "string") {
                ...
                connection.SendMessage(
                    "Emote",
                    `*${target} has been banned by administrator.`,
                );
                return target.Ban();
            }
        });

        this.registerSUCommand(
            "resize",
            (connection, argv, sender, respond) => {
                if (argv.length !== 1) {
        ...
        this.registerSUCommand(
            ...
            (connection, argv, sender, respond) => {
                ...
                return connection.ChatRoomUpdate({
                    Visibility:
                        argv[0] === "true" ? ["Admin", "Whitelist"] : ["All"],
                });
            },
        );
     ...
     *                    command list, either because it is an alias for another command using the same callback or it is a hidden one.
     */
    protected registerCommandParsed(
        name: string,
        callback: AdminCommandHandlerParsed,
        description: string | null = "",
    ) {
    ...
    private a_command_su(
        connection: API_Connector,
        argv: string[],
        sender: number,
        respond: (response: string) => void,
    ) {
        if (!this.config.superusers.includes(sender)) {
            ...
            `Available commands:\n` +
                Array.from(this.a_commands.entries())
                    ...
                    .map(
                        (c) =>
                            `!${c[0]}` +
                            (c[1].description ? ` - ${c[1].description}` : ""),
                    )
                    .sort()
                    .join("\n"),
        );
        ...
        return new Promise((resolve) => {
            const participants = (
                options.participants
                    ? options.participants.filter((p) =>
                          room.characters.includes(p),
                      )
                    : room.characters.slice()
            ).filter((p) => !p.IsBot());
    ...
    private a_CharacterVote(
        ...
        if (!args.trim()) {
            const votes = Array.from(this.a_pendingVotes.entries()).filter(
                (i) => i[1].votes.has(sender),
            );
            if (votes.length === 0) {
                sender.Tell(
                    "Whisper",
                    "There are currently no active votes you can vote on",
                );
            }
            if (votes.length === 1) {
                args = votes[0][0];
        ...
        if (!voteData) {
            sender.Tell(
                "Whisper",
                "No such vote found. To see currently ongoing votes, use '!listvotes'\n" +
        ...
        if (res !== null) {
            sender.Tell(
                "Whisper",
                `You already voted ${res ? "yes" : "no"} in this vote. Votes cannot be changed.`,
            );
            return;
        }
        ...
        if (
            !end &&
            Array.from(voteData.votes.values()).every((v) => v !== null)
        ) {
            end = true;
        }
        if (end) {
            ...
            connection.SendMessage(
                "Chat",
                `Vote "${voteData.name}" update:\nYes: ${results.yes}  No: ${results.no}`,
            );
        }
    }

    private a_command_listVotes(
        connection: API_Connector,
        args: string,
        sender: API_Character,
    ) {
        ...
        sender.Tell(
            ...
            `List of currently ongoing votes: \n` +
                Array.from(this.a_pendingVotes.values())
                    .map(
                        (v) =>
                            `> "${v.name}" ${v.description ? `- ${v.description} ` : ""}(${Math.floor((v.endTime - Date.now()) / 1000)}s left)`,
                    )
                    .join("\n") +
                (Array.from(this.a_pendingVotes.values()).some(
                    (v) => v.votes.get(sender) === null,
                )
    ...
    protected identifyPlayerInRoom(
        room: API_Chatroom,
        identifier: string,
        ...
        if (targets.length === 0)
            targets = room.characters.filter(
                (c) =>
                    c.Name.toLocaleLowerCase() ===
                    identifier.toLocaleLowerCase(),
            );
        if (targets.length === 0)
            targets = room.characters.filter((c) =>
                c.Name.toLocaleLowerCase().startsWith(
                    identifier.toLocaleLowerCase(),
                ),
            );
    ...
    private a_command_kick(
        ...
        if (!targetMatch) {
            sender.Tell(
                "Whisper",
                `Expected format: !${ban ? "ban" : "kick"} <target> [Reason]`,
            );
            return;
        }
        const target = this.identifyPlayerInRoom(
        ...
        if (target === sender) {
            sender.Tell(
                "Whisper",
                "You know you can just leave the room, right?",
            );
            return;
        }
        ...
        const canBeBannedUntil = this.a_banAvailability.get(
            target.MemberNumber,
        );
        if (canBeBannedUntil != null && canBeBannedUntil >= Date.now()) {
            if (
                !ban &&
                this.a_notBanComfirmation.get(sender) !== target.MemberNumber
            ) {
                sender.Tell(
                    "Whisper",
                    `Note:\n` +
    ...
    private a_start_kickvote(
        ...
        if (this.a_pendingKickVotes.has(target.MemberNumber)) return;

        const voteName = `${ban ? "ban" : "kick"} ${target.MemberNumber}`;
        this.a_kickProtection.set(
            target,
        ...
        this.startVote(voteName, target.chatRoom, KICKVOTE_DURATION, {
            ...
            autoVoteNo: [target],
            reportProgress: true,
            progress: ({ yes, no, didNotVote }) =>
                yes > no + didNotVote || no >= yes + didNotVote,
            startHook: (name) => {
                logger.alert(
                    `${this.a_LogHeader(connection)} Vote to ${ban ? "ban" : "kick"} ${target} started by ${sender} (vote id: ${name})`,
                );
            ...
            logger.alert(
                `${this.a_LogHeader(connection)} Vote to ${ban ? "ban" : "kick"} ${target} (vote id: ${result.name}) ended with result Y: ${result.yes}  N: ${result.no}  DNV: ${result.didNotVote}`,
            );
    ...
    //#region Room guard
    private a_guard_givePoints(
        target: API_Character,
        points: number,
        reason: string,
    ) {
        if (!this.a_settings.roomGuardEnabled) return;
        if (!(points > 0)) {
            logger.error(
                `${this.a_LogHeader(target.connection)} Non-positive number of points for guard to give`,
                points,
                new Error(),
            );
            return;
        }
        if (target.IsRoomAdmin()) return;
        ...
        metric_guard_points_current
            .labels({ memberNumber: target.MemberNumber })
            .set(data.points);
        metric_guard_points
            .labels({ memberNumber: target.MemberNumber })
            .inc(points);
        if (
            ...
            !this.a_guard_didWarn.has(target.MemberNumber)
        ) {
            logger.alert(
                `${this.a_LogHeader(target.connection)} GuardPoints warning for ${target}: ${data.points}\n`,
                ...data.log,
            );
            data.points = ROOMGUARD_THRESHOLD_WARN;
            ...
            data.points >= ROOMGUARD_THRESHOLD_ACTION &&
            !this.a_guard_Acted.has(target.MemberNumber)
        ) {
            const canBeBannedUntil = this.a_banAvailability.get(
                target.MemberNumber,
            );
            ...
            logger.alert(
                `${this.a_LogHeader(target.connection)} RoomGuard kicked ${target}: ${data.points}\n`,
                ...data.log,
            );
            target.connection.SendMessage(
                "Emote",
                `*${target} has been automatically kicked by Room Guard™, as their actions have been detected as likely disruptive.`,
            );
            target.Kick();
...
*/
    ...
    private a_guard_pointDecay() {
        for (const [memberNumber, data] of this.a_guard_points.entries()) {
            ...
            if (
                (this.a_guard_didWarn.has(memberNumber) ||
                    this.a_guard_Acted.has(memberNumber)) &&
                data.points <= ROOMGUARD_THRESHOLD_OK
            ) {
                logger.alert(
                    `[A] Room guard warning reset for ${memberNumber}`,
                );
        ...
        if (this.a_settings.inactivityKickTimer !== null) {
            for (const [
                ...
                if (
                    ...
                    now >=
                        lastActivity + this.a_settings.inactivityWarningTimer &&
                    ...
                    this.a_lastActivity.set(
                        character,
                        now - this.a_settings.inactivityWarningTimer,
                    );
                } else if (
                    now >=
                    lastActivity + this.a_settings.inactivityKickTimer
                ) {
                    character.connection.SendMessage(
                        "Emote",
                        `*${character} has been kicked due to inactivity.`,
                    );
                    void character.Kick();
                }
            }
    ...
    private a_LogHeader(connection: API_Connector): string {
        return (
            ...
            (this.a_settings.logConnectionMemberNumber
                ...
                : "") +
            (this.a_settings.logConnectionName
                ? ` [${connection.username}]`
                : "")
        );
    }
    ...
    private a_onMessage(event: LogicEvent_Message): boolean {
        if (this.a_settings.log) {
            const dict =
                event.message.Dictionary === undefined
                    ? ""
                    : `; dict: ${JSON.stringify(event.message.Dictionary)}`;
            if (event.message.Type !== "Hidden") {
                const msg =
                    `${this.a_LogHeader(event.connection)} Message ${event.message.Type} ` +
                    `from ${event.Sender.Name} (${event.Sender.MemberNumber}): ` +
                    `${event.message.Content}${dict}`;
                if (
                    event.Sender.IsBot() ||
                    (event.message.Type === "Action" &&
                        ["ActionUse", "ActionRemove"].includes(
                            event.message.Content,
                        ))
                ) {
                    logger.debug(msg);
                } else if (
                    ["Chat", "Emote", "Whisper"].includes(event.message.Type)
                ) {
                    logger.info(msg);
                } else {
                    logger.verbose(msg);
        ...
        if (event.message.Type === "Chat" || event.message.Type === "Emote") {
            let points = _.sum(
                event.message.Content.split("\n").map((line) =>
                    Math.max(1, Math.ceil(line.length / ROOMGUARD_CHAT_WIDTH)),
                ),
            );
            if (
                this.a_guard_lastMessage.get(event.Sender) ===
                event.message.Content
            ) {
                points *= 2;
            } else {
                this.a_guard_lastMessage.set(
                    event.Sender,
                    event.message.Content,
                );
            }
        ...
        if (
            event.message.Type === "Whisper" &&
            event.message.Content.startsWith("!")
        ) {
            const commandMatch = /^!\s*(\S+)(?:\s|$)(.*)$/.exec(
                event.message.Content,
            );
            if (commandMatch) {
            ...
            const byStr =
                event.sourceMemberNumber !== undefined
                    ? resolvedCharacter !== undefined
                        ? ` by ${resolvedCharacter.Name} (${resolvedCharacter.MemberNumber})`
                        : ` by (${event.sourceMemberNumber})`
                    : "";
            logger.info(
                `${this.a_LogHeader(event.connection)} Left: ` +
                    `${event.character.Name} (${event.character.MemberNumber}), ` +
                    `reason: ${event.leaveMessage}` +
                    byStr,
            );
        }
    ...
    private a_onCharacterEntered(event: LogicEvent_CharacterEntered): boolean {
        if (this.a_settings.log) {
            logger.info(
                `${this.a_LogHeader(event.connection)} Entered: ` +
                    `${event.character.Name} (${event.character.MemberNumber})`,
            );
            const curseVersion = event.character.getCurseVersion();
            if (curseVersion !== null) {
                logger.verbose(
                    `${this.a_LogHeader(event.connection)} ${event.character.Name} (${event.character.MemberNumber})` +
        ...
        for (const vote of this.a_pendingVotes.values()) {
            const oldCharacter = Array.from(vote.votes.keys()).find(
                (c) => c.MemberNumber === event.character.MemberNumber,
            );
            if (oldCharacter) {
    ...
    private a_onCharacterEvent(
        ...
        if (event.name === "ItemAdd" && !event.initial) {
            ...
            logger.alert(
                `${this.a_LogHeader(connection)} ${event.character.Name} (${event.character.MemberNumber}) used swafeword! (full release: ${event.release})`,
            );
        }
    ...
    private a_onBeep(event: LogicEvent_Beep): boolean {
        if (
            typeof event.beep.BeepType === "string" &&
            !event.beep.BeepType.startsWith("{")
        ) {
        ...
        if (
            event.beep.BeepType == null &&
            typeof event.beep.Message === "string" &&
            this.config.superusers.includes(event.beep.MemberNumber)
        ) {
            for (const pfx of ["!admin ", "!su ", "!sudo "]) {
                if (event.beep.Message.toLocaleLowerCase().startsWith(pfx)) {
                    const argv = [
                        ...event.beep.Message.substr(pfx.length).matchAll(
                            /".+?(?:"|$)|'.+?(?:'|$)|[^ ]+/g,
                        ),
                    ]
                        .map((a) => a[0])
    ...
    private a_onBotEvent(
        connection: API_Connector,
        event: AnyBotEvent,
    ): boolean {
        if (event.name === "Message" && this.a_settings.log) {
            const dict =
                event.Dictionary == null
                    ? ""
            ...
            const target =
                event.Target === null ? "" : `to ${tc || event.Target} `;
            if (event.Type !== "Hidden") {
                const msg =
                    `${this.a_LogHeader(connection)} Bot message ${event.Type} ` +
                    target +
                    `: ` +
                    `${event.Content}${dict}`;
                logger.debug(msg);
