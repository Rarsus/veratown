...
const club_member_intro =
    `GAME: You are one of the club members. Your task is to observe and ask around, in order to ` +
...
export class KidnappersGameRoom extends AdministrationLogic {
    ...
    roleWarnings: Set<"kidnappers" | "maid" | "stalker" | "mistress"> =
        new Set();
    ...
    private charTimer: Map<API_Character, ReturnType<typeof setTimeout>> =
        new Map();

    readonly conn: API_Connector;

    constructor(conn: API_Connector, config: ConfigFile) {
        super({ inactivityKickEnabledOnlyBelowFreeSlotsCount: 6 }, config);
        this.conn = conn;

        this.registerCommand(
            "status",
            (connection, args, sender) => this.handleStatusCommand(sender),
            `To get information about the running game`,
        );
        this.registerCommand(
            "joingame",
            (connection, args, sender) => this.handleJoingameCommand(sender),
            `To register as a new player`,
        );
        this.registerCommand(
            "freeandleave",
            (connection, args, sender) =>
                this.handleFreeandleaveCommand(sender),
            `To urgently leave. Will untie and kick you`,
        );
        this.registerCommand(
            "start",
            (connection, args, sender) => this.handleStartCommand(sender),
            `To start a new round after registering`,
        );
        this.registerCommand(
            "skip",
            (connection, args, sender) => this.handleSkipCommand(sender),
            `To propose ending a day phase early`,
        );
        this.registerCommand(
            "suspect",
            (connection, args, sender) =>
                this.handleSuspectCommand(args, sender),
            `To suspect someone during the day`,
        );
        this.registerCommand(
            "guilty",
            (connection, args, sender) => this.handleGuiltyCommand(sender),
            `To vote against a suspect during a day time vote`,
        );
        this.registerCommand(
            "innocent",
            (connection, args, sender) => this.handleInnocentCommand(sender),
            `To believe in a suspect during a day time vote`,
        );
        this.registerCommand(
            "kidnap",
            (connection, args, sender) =>
                this.handleKidnapCommand(args, sender),
            null,
        );
        this.registerCommand(
            "watch",
            (connection, args, sender) => this.handleWatchCommand(args, sender),
            null,
        );
        this.registerCommand(
            "stalk",
            (connection, args, sender) => this.handleStalkCommand(args, sender),
            null,
        );
        this.registerCommand(
            "protect",
            (connection, args, sender) =>
                this.handleProtectCommand(args, sender),
            null,
        );
        this.registerCommand(
            "beepme",
            (connection, args, sender) => this.handleBeepmeCommand(sender),
            `To get beeped when enough players are online`,
        );
        this.registerCommand(
            "myrole",
            (connection, args, sender) => this.handleMyroleCommand(sender),
            `To show only you your assigned role again`,
        );
    ...
    playerGreeting(character: API_Character) {
        ...
        character.Tell(
            "Emote",
            `*a social deduction game to some maybe known as 'Mafia' or 'Werewolf' ` +
        ...
        character.Tell(
            "Chat",
            `NEWS: There is a new command !myrole to whisper you your role once more during a running game.`,
        );
    }
...
KIDNAPPERS - The Game
===================================================================
by D. Davidoff / A. Plotkin, room concept by Claudia & Clare, using ropeybot.
...
► !start                  Starts the game for everyone! Needs 5+ players
...
In urgent cases, you can also contact EllieThePink, on BondageClub discord.

This bot is made with ropeybot, from Jomshir's original BotAPI code. Fixes and improvements welcome!
...
`.trim();
    ...
    protected async onCharacterEntered(
        ...
        const oldIndex = this.players.findIndex(
            (p) => p.MemberNumber === character.MemberNumber,
        );
        if (oldIndex >= 0) {
            // old player returning
            const oldCharacter = this.players[oldIndex];

            this.charTimer.forEach(
                (value, key) =>
                    key.MemberNumber === character.MemberNumber &&
                    clearTimeout(value),
            );
            ...
            if (this.club_members.has(oldCharacter)) {
                ...
                if (this.suspicions.has(oldCharacter)) {
                    this.suspicions.set(
                        character,
                        this.suspicions.get(oldCharacter)!,
                    );
                    this.suspicions.delete(oldCharacter);
                }
                this.suspicions.forEach((key, value) => {
                    if (value === oldCharacter)
                        this.suspicions.set(key, character);
                });
            }
    ...
    private unregisterPlayer(character: API_Character, message: string) {
        if (this.players.includes(character)) {
            this.conn.SendMessage(
                "Emote",
                `*GAME: ${character} was unregistered as an active player` +
                    message,
            );
    ...
    private resolvePlayerRemovals(character: API_Character) {
        ...
        if (
            this.kidnappers.size === 0 &&
            this.gameState !== "game_not_started" &&
            this.gameState !== "game_was_won"
        ) {
            this.clubMembersWin();
        }
    }
    ...
    protected onMessage(
        ...
        if (message.Type === "Chat") {
            if (
                msg.startsWith("!suspect") &&
                this.gameState === "waiting_on_day_activities" &&
                this.club_members.has(sender)
            ) {
                if (!this.active_config.openSuspicions) {
                    sender.Tell(
                        "Whisper",
                        `GAME: In the configuration for this game round, suspicions need to be whispered to the bot.`,
                    );
                } else {
    ...
    handleStatusCommand(sender: API_Character) {
        if (
            ...
            sender.Tell(
                "Whisper",
                `GAME: Start the next game with !start when enough players have registered using !joingame` +
            ...
            sender.Tell(
                "Whisper",
                `GAME: This is the current status of the game:` +
                    `\nThe remaining participants in the current round are:\n` +
                    Array.from(this.club_members.values())
                        .map((A) => A.toString())
                        .join(", "),
            );
        }
    ...
    async handleJoingameCommand(sender: API_Character) {
        ...
        if (!sender.ProtectionAllowInteract()) {
            sender.Tell(
                "Chat",
                `Warning: You are unable to join the game because the bot currently cannot interact with you ` +
            ...
            logger.info(
                `Player check for ${sender}: Protection: Version=${sender.OnlineSharedSettings.GameVersion}, Admin=${this.conn.Player.IsRoomAdmin()}`,
            );
            return;
        }
        ...
        if (!allow) {
            sender.Tell(
                "Chat",
                `Warning: You are unable to join the game because the bot cannot interact with you due to ` +
            ...
            logger.info(
                `Player check for ${sender}: Permission level: ` +
                    BC_PermissionLevel[sender.ItemPermission],
            );
            return;
        }
        ...
        this.conn.SendMessage(
            "Emote",
            `*GAME: ${sender} was registered as an active player.`,
        );

        this.beepSuccess =
            await this.matchmaking_notifier.notifyPlayersOfEnoughInterest(
                this.players,
            );
        if (this.beepSuccess) {
            sender.Tell(
                "Chat",
                `GAME: You joining the game triggered a matchmaking beep to ${BEEP_AT_THIS_COUNT} players in ` +
            ...
            sender.Tell(
                "Chat",
                `GAME: There ${this.matchmaking_notifier.waitingPlayers > 1 ? `are` : `is`} currently ` +
    ...
    handleStartCommand(sender: API_Character) {
        if (!this.players.includes(sender)) {
            sender.Tell(
                "Whisper",
                "GAME: Please register first by writing '!joingame' into the chat.",
            );
        } else if (this.gameState !== "game_not_started") {
            sender.Tell(
                "Whisper",
                "GAME: Cannot start, as a game is currently already in progress. You will be part of the next one.",
            );
        } else if (
            this.players.includes(sender) &&
            this.gameState === "game_not_started"
        ) {
            ...
            if (this.players.length < 5) {
                this.conn.SendMessage(
                    "Emote",
                    `*GAME: The game needs a minimum of five registered players to start the next round.`,
                );
                return;
            ...
            this.conn.SendMessage(
                "Emote",
                `*GAME: It's on! The league started the event. It might be a good idea to take ` +
            ...
            this.conn.SendMessage(
                "Emote",
                `*GAME: This round is played with the game configuration '${config}' ` +
                    `(see bot's profile / online description)`,
            );
            if (this.active_config.discloseRolesAtStart) {
                this.conn.SendMessage(
                    "Emote",
                    `*GAME: The following secret roles are present:` +
    ...
    handleSkipCommand(sender: API_Character) {
        if (
            (this.gameState === "waiting_on_trial_votes" ||
                this.gameState === "waiting_on_accused_defense" ||
                this.gameState === "waiting_on_day_activities" ||
                this.gameState === "day_1") &&
            this.club_members.has(sender) &&
            !this.skip_day.has(sender)
        ) {
            if (this.gameState === "waiting_on_day_activities") {
                this.skip_day.add(sender);
                this.conn.SendMessage(
                    "Emote",
                    `*GAME: ${sender} seems to get ready for an early night.`,
                );
            } else {
                sender.Tell(
                    "Whisper",
                    "GAME: You cannot use this right now. Please wait until after the voting.",
                );
            }
            ...
            sender.Tell(
                "Whisper",
                "GAME: You already have voted in favor of ending today early.",
            );
        } else {
            sender.Tell(
                "Whisper",
                "GAME: You can only use this command during the day phase of a running game.",
            );
        }
    }
    ...
    async handleBeepmeCommand(sender: API_Character) {
        if (this.blockMatchmakingJoin) {
            sender.Tell(
                "Whisper",
                `GAME: The beepme command is temporarily deactivated, since the previous 'matchmaking' ` +
            ...
            await this.matchmaking_notifier.addPlayerToTheMatchmakingQueue(
                sender,
            );
            this.beepSuccess =
                await this.matchmaking_notifier.notifyPlayersOfEnoughInterest(
            ...
            sender.Tell(
                "Whisper",
                `GAME: You cannot use this during a running game. Please register as a player ` +
    ...
    handleGuiltyCommand(sender: API_Character) {
        if (
            ...
            this.conn.SendMessage(
                "Emote",
                `*GAME: ${sender} found ${this.the_accused} to be GUILTY!`,
            );
            this.innocent_votes.delete(sender);
            if (this.guilty_votes.size / this.club_members.size > 0.5) {
                this.conn.SendMessage(
                    "Emote",
                    `*GAME: The club members have cast their vote on ${this.the_accused} and the ` +
                ...
                this.guilty_votes.size + this.innocent_votes.size ===
                this.club_members.size
            ) {
                this.conn.SendMessage(
                    "Emote",
                    `*GAME: The club members have cast their vote and ${this.the_accused} ` +
    ...
    handleInnocentCommand(sender: API_Character) {
        if (
            ...
            this.conn.SendMessage(
                "Emote",
                `*GAME: ${sender} found ${this.the_accused} to be INNOCENT!`,
            );
            if (
                this.guilty_votes.size + this.innocent_votes.size ===
                this.club_members.size
    ...
    private handleMyroleCommand(sender: API_Character) {
        if (this.players.find((e) => e === sender) === undefined) {
            sender.Tell(
                "Whisper",
                `GAME: You are not part of the currently running game as you did not yet register as a player.`,
            );
            return;
        }

        this.sendRoleMessage(sender);
    }
    ...
    handleKidnapCommand(msg: string, sender: API_Character) {
        if (this.kidnappers.has(sender)) {
            if (
                ...
                (this.gameState === "waiting_on_night_activities" ||
                    (this.gameState === "night_1" &&
                        this.active_config.firstNightKidnapping))
            ) {
                ...
                if (!match) {
                    sender.Tell(
                        "Whisper",
                        `GAME: Bad format, expected '!kidnap [name OR memberID]' for example !kidnap ${sender.Name} or ` +
                ...
                if (isInteger) {
                    ...
                    this.club_members.forEach(
                        (item) => item.MemberNumber === i && list.push(item),
                    );
                } else {
                    const i = match[1].toLocaleLowerCase();
                    this.club_members.forEach(
                        (item) =>
                            item.Name.toLocaleLowerCase() === i &&
                            list.push(item),
                    );
                }
                if (list.length > 1) {
                    sender.Tell(
                        "Whisper",
                        `GAME: The player name is not unique, please use the member number of the person instead, ` +
                            `for example !kidnap ${sender.MemberNumber}`,
                    );
                } else if (list.length > 0) {
                    this.kidnappers_target = list[0];
                    const kidnappers_team: API_Character[] = Array.from(
                        this.kidnappers,
                    );
                    ...
                    if (kidnappers_team.length === 2) {
                        ...
                        logger.error(
                            `Unexpected value for kidnapper's team size: ${kidnappers_team.length}`,
                        );
                    }
                    for (const i of tmp_array) {
                        if (
                            this.kidnappers_target.MemberNumber ===
                            kidnappers_team[i].MemberNumber
                        ) {
                            sender.Tell(
                                "Whisper",
                                `GAME: No kidnapper kidnaps one of their own!`,
                            );
                            this.kidnappers_target = null;
                            return;
                        }
                    }
                    this.whisperRemainingKidnappers(
                        `You are moving in, preparing to kidnap ${this.kidnappers_target.Name}`,
                    );
                } else {
                    sender.Tell(
                        "Whisper",
                        `GAME: This player name or member number is not recognized! Please try again.`,
                    );
                }
            } else {
                sender.Tell(
                    "Whisper",
                    `GAME: You cannot kidnap someone ${this.kidnappers_target === null ? `` : `else `}` +
    ...
    handleWatchCommand(msg: string, sender: API_Character) {
        if (
            ...
            (this.gameState === "waiting_on_night_activities" ||
                this.gameState === "night_1") &&
            this.club_members.has(this.maid)
        ) {
            let isInteger: boolean = true;
            const list: API_Character[] = [];
            let match = /^([0-9]+)$/i.exec(msg);
            ...
            if (!match) {
                sender.Tell(
                    "Whisper",
                    `GAME: Bad format, expected '!watch [name OR memberID]' for example !watch ${sender.Name} or ` +
            ...
            if (isInteger) {
                ...
                this.club_members.forEach(
                    (item) =>
                        item.Name.toLocaleLowerCase() === i && list.push(item),
                );
            }
            if (list.length > 1) {
                sender.Tell(
                ...
                if (this.maid_target.MemberNumber === this.maid.MemberNumber) {
                    sender.Tell(
                        "Whisper",
                        `GAME: You decided to stare at your mirror image all night long. ` +
                            `You are definately not a kidnapper.`,
                    );
                } else {
                    sender.Tell(
                        "Whisper",
                        `GAME: You watched ${this.maid_target.Name} all night long, ` +
                            `even sneaking into their room with your maid cleaning service general key ` +
                            `${
                                this.kidnappers.has(this.maid_target)
                                    ? "and eventually spotted her trying to kidnap an unsuspecting " +
                                      `club member! You quickly retreat, your heart beating fast. She definately is a ` +
                                      `kidnapper!`
                                    : `, but could find no signs that she ever has or will do some backhanded kidnapping, ` +
    ...
    handleStalkCommand(msg: string, sender: API_Character) {
        if (
            ...
            (this.gameState === "waiting_on_night_activities" ||
                this.gameState === "night_1") &&
            this.club_members.has(this.stalker)
        ) {
            let isInteger: boolean = true;
            const list: API_Character[] = [];
            let match = /^([0-9]+)$/i.exec(msg);
            ...
            if (!match) {
                sender.Tell(
                    "Whisper",
                    `GAME: Bad format, expected '!stalk [name OR memberID]' for example !stalk ${sender.Name} or ` +
            ...
            if (list.length > 1) {
                ...
                if (
                    this.stalker_target.MemberNumber ===
                    this.stalker.MemberNumber
                ) {
                    sender.Tell(
                        "Whisper",
                        `GAME: You decided to not stalk anyone tonight.`,
                    );
                } else {
                    sender.Tell(
                        "Whisper",
                        `GAME: You stalked ${this.stalker_target.Name} all night long, ` +
                            `${
                                this.maid === this.stalker_target
                                    ? "while grinning happily, as she is your beloved shy maid in the flesh!"
    ...
    handleProtectCommand(msg: string, sender: API_Character) {
        if (
            this.mistress === sender &&
            this.mistress_target === null &&
            (this.gameState === "waiting_on_night_activities" ||
                this.gameState === "night_1") &&
            this.club_members.has(this.mistress)
        ) {
            let isInteger: boolean = true;
            const list: API_Character[] = [];
            let match = /^([0-9]+)$/i.exec(msg);
            ...
            if (!match) {
                sender.Tell(
                    "Whisper",
                    `GAME: Bad format, expected '!protect [name OR memberID]' for example !protect ${sender.Name} or ` +
            ...
            if (list.length > 1) {
                ...
                if (
                    this.mistress_last_target === list[0] &&
                    !this.active_config.mistressCanpickSameTargetTwice
                ) {
                    sender.Tell(
                        "Whisper",
                        `GAME: You don't feel like doing the same thing as last night again. ` +
                ...
                if (
                    this.mistress_target.MemberNumber ===
                    this.mistress.MemberNumber
                ) {
                    if (this.active_config.mistressCanProtectHerself) {
                        sender.Tell(
                            "Whisper",
                            `GAME: You spent the night in full mistress gear, your bull whip in hand, ` +
                                `while sitting on your throne, hoping a kidnapper would visit you. No one dared showing up though.`,
                        );
                    } else {
                        sender.Tell(
                            "Whisper",
                            `GAME: You really don't feel like staying up all night, just for the off chance that ` +
                    ...
                    sender.Tell(
                        "Whisper",
                        `GAME: You summoned the lovely ${this.mistress_target.Name} to your room and spent most of ` +
    ...
    handleSuspicion(msg: string, sender: API_Character) {
        ...
        if (!match) {
            sender.Tell(
                "Whisper",
                `GAME: Bad format, expected '!suspect [name OR memberID]' for example !suspect ${sender.Name} or ` +
        ...
        if (list.length > 1) {
            ...
            if (this.suspicions.has(list[0])) {
                if (this.suspicions.get(list[0]) === sender) {
                    sender.Tell(
                        "Whisper",
                        `GAME: You already voiced a suspicion against ${list[0].Name}.`,
                    );
                } else {
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*GAME: ${this.the_accused} is suspected of being a kidnapper` +
                    ...
                    const rope = list[0].Appearance.AddItem(
                ...
                this.suspicions.forEach(
                    (value, key) =>
                        value.MemberNumber === sender.MemberNumber &&
                        this.suspicions.delete(key),
                );
                this.suspicions.set(list[0], sender);
                if (list[0] === sender && this.active_config.openSuspicions) {
                    this.conn.SendMessage(
                        "Emote",
                        `*GAME: A suspicion against herself has been voiced by ${sender}! ` +
                            `Why would she do something crazy like that?`,
                    );
                } else {
                    this.conn.SendMessage(
                        "Emote",
                        `*GAME: A suspicion against ${list[0]} has been voiced ${
                            this.active_config.openSuspicions
                                ? `by ${sender}`
                                : ``
                        }! Is she really a kidnapper?`,
                    );
                }
            }
        } else {
    ...
    private freeAndGiveTrialStats() {
        const no_vote_list = Array.from(this.club_members.values()).filter(
            (A) => !this.guilty_votes.has(A) && !this.innocent_votes.has(A),
        );
        ...
        this.conn.SendMessage(
            "Emote",
            `*GAME: These club members voted guilty:\n` +
                Array.from(this.guilty_votes.values())
                    .map((A) => A.toString())
                    .join(", ") +
                `\nThese club members voted innocent:\n` +
                Array.from(this.innocent_votes.values())
                    .map((A) => A.toString())
                    .join(", ") +
    ...
    private Tick() {
        ...
        if (
            !this.roleWarnings.has("mistress") &&
            now + 25_000 >= this.turnTimer &&
            this.mistress_target === null &&
            (this.gameState === "waiting_on_night_activities" ||
                this.gameState === "night_1")
        ) {
            if (this.mistress !== null) {
                this.mistress.Tell(
                    "Whisper",
                    `GAME: Psssst... only 25 seconds left to make night plans, glorious mistress !`,
                );
                this.roleWarnings.add("mistress");
            }
        }
        if (
            !this.roleWarnings.has("maid") &&
            now + 25_000 >= this.turnTimer &&
            this.maid_target === null &&
            (this.gameState === "waiting_on_night_activities" ||
                this.gameState === "night_1")
            ...
            if (this.maid !== null) {
                this.maid.Tell(
                    "Whisper",
                    `GAME: Psssst... only 25 seconds left to invstigate a club member, sneaky maid !`,
                );
                this.roleWarnings.add("maid");
            }
        }
        if (
            !this.roleWarnings.has("stalker") &&
            now + 25_000 >= this.turnTimer &&
            this.stalker_target === null &&
            (this.gameState === "waiting_on_night_activities" ||
                this.gameState === "night_1")
            ...
            if (this.stalker !== null) {
                this.stalker.Tell(
                    "Whisper",
                    `GAME: Psssst... only 25 seconds left to find the maid, love-struck stalker !`,
                );
                this.roleWarnings.add("stalker");
            }
        }
        if (
            !this.roleWarnings.has("kidnappers") &&
            now + 25_000 >= this.turnTimer &&
            this.kidnappers_target === null &&
            (this.gameState === "waiting_on_night_activities" ||
                this.gameState === "night_1")
            ...
            this.whisperRemainingKidnappers(
                "Psssst... only 25 seconds left to kidnap a club member",
            );
		...
		*/
        ...
        if (
            ...
            (this.mistress === null ||
                ...
                this.mistress_target !== null) &&
            (this.maid === null ||
                (this.maid !== null && !this.club_members.has(this.maid)) ||
                this.maid_target !== null) &&
            (this.stalker === null ||
                (this.stalker !== null &&
                    !this.club_members.has(this.stalker)) ||
            ...
            if (this.kidnappers_target !== null) {
                if (
                    ...
                    this.tieUpCharacterAndRemove(
                        "kidnapper",
                        this.kidnappers_target,
                    );
                    this.conn.SendMessage(
                        "Emote",
                        `*GAME: The sun starts rising and the club members slowly realize that ` +
                            `${this.kidnappers_target.Name} is missing. Another kidnapping! Who is it?`,
                    );
                } else if (
                    this.active_config.announceMistressProtectionSucess
                ) {
                    this.conn.SendMessage(
                        "Emote",
                        `*GAME: The sun starts rising - it's a lovely new day! The dashing mistress ` +
                    ...
                    this.conn.SendMessage(
                        "Emote",
                        `*GAME: The sun starts rising - it's a lovely new day! There was no kidnapping last night!`,
                    );
                }
                this.setGameState("waiting_on_day_activities");
                ...
                this.conn.SendMessage(
                    "Emote",
                    `*GAME: The sun starts rising - it's a lovely new day! The kidnappings seemingly did not start yet.`,
                );
                this.setGameState("waiting_on_day_activities");
                this.giveDayInstructions();
            }
        ...
        if (now >= this.turnTimer) {
            if (this.gameState === "day_1") {
                ...
                if (this.gameState === "waiting_on_accused_defense") {
                    this.conn.SendMessage(
                        "Emote",
                        `*GAME: It got too late so the club members decided to let ${this.the_accused} go for today.`,
                    );
                    this.the_accused?.Appearance.RemoveItem("ItemArms");
                    ...
                    this.freeAndGiveTrialStats();
                    logger.warning(
                        "Reached stat that should not have been possible: The day ended early, during a running voting.",
                    );
                }
                this.setGameState("waiting_on_night_activities");
                this.giveNightInstructions();
                ...
                this.conn.SendMessage(
                    "Emote",
                    `*GAME: The next game can now by started by any registered player with '!start'. ` +
        ...
        if (
            ...
            this.conn.SendMessage(
                "Emote",
                `*GAME: 20 seconds left to cast a vote.`,
            );
        }
        ...
        if (
            ...
            if (this.gameState === "waiting_on_accused_defense") {
                ...
                this.conn.SendMessage(
                    "Emote",
                    `*GAME: The club members have cast their vote and ${this.the_accused} was declared innocent and freed.`,
                );
                this.freeAndGiveTrialStats();
                this.setGameState("waiting_on_day_activities");
            }
    ...
    private updateSign() {
        ...
        if (sign) {
            ...
            const dayActivityTimeLeft = Math.ceil(
                Math.max(0, this.dayTimer - now) / 1000,
            );
    ...
    giveNightInstructions() {
        ...
        if (this.club_members.size === 2 && this.kidnappers.size === 2) {
            ...
            this.club_members.size === 2 &&
            this.switch !== null &&
            this.club_members.has(this.switch)
        ) {
            this.conn.SendMessage(
                "Emote",
                `*GAME: Now the switch is all alone, face to face with the kidnapper, who is smirking at ` +
        ...
        this.conn.SendMessage(
            "Emote",
            `*GAME: Night falls and all participants slowly walk back to their rooms, sleepy after ` +
                `today's events.`,
        );
        ...
        if (
            ...
            this.whisperRemainingKidnappers(
                "You are using tonight to prepare tomorrow night's kidnapping thoroughly." +
        ...
        if (this.mistress !== null && this.club_members.has(this.mistress)) {
            this.mistress.Tell(
                ...
                `GAME - WHISPER COMMAND NEEDED:\n` +
                    `${
                        this.active_config.mistressCanProtectHerself
                            ? `You can now decide to either protect yourself from being kidnapped or `
                            : `You can now `
                    }` +
    ...
    giveDayInstructions() {
        ...
        this.conn.SendMessage(
            "Emote",
            `*${this.active_config.openSuspicions ? `GAME:` : `GAME - WHISPER COMMAND NEEDED:\n`}` +
    ...
    giveTrialInstruction() {
        this.conn.SendMessage(
            "Emote",
            `*GAME: After giving ${this.the_accused} ample time to refute the accusations, ` +
    ...
    setAllRolesAndCommunicate() {
        ...
        this.gameEndMessage =
            ...
            `A round of applause ` +
            `also for the other wonderful club members: ` +
            remainingPlayerSelection.map((C) => C.toString()).join(", ");
    }
    ...
    tieUpCharacterAndRemove(
        who: "club" | "kidnapper" | "club revenge",
        character: API_Character,
    ) {
        ...
        if (who === "kidnapper" || who === "club revenge") {
            ...
            if (who === "club revenge") {
                character.Tell(
                    "Whisper",
                    `GAME: The club members approached you and tied you up even stricter, making sure ` +
                ...
                character.Tell(
                    "Whisper",
                    `GAME: When you realized what the kidnapper was trying to do, it was already too late. ` +
            ...
            const leg_rope = character.Appearance.AddItem(
                AssetGet("ItemLegs", "HempRope"),
            );
            leg_rope?.Extended?.SetType("Frogtie");
        }
        rope?.SetDifficulty(20);
        const gag1 = character.Appearance.AddItem(
        ...
        const gag3 = character.Appearance.AddItem(
        ...
        for (const g of [gag1, gag2, gag3]) {
            if (who === "kidnapper") {
                g.SetCraft({
                    Name: `Kidnapping Gag`,
                    Description: `For professional kidnappers: when you need your victim completely silent!`,
                    Property: "Large",
                });
            } else {
                g.SetCraft({
                    Name: `Anti-kidnapper Gag`,
                    Description: `You stand accused of the most heinous crime of kidnapping! What do you have to say for yourself? Nothing, it seems.`,
                    Property: "Large",
                });
            }
        }
    ...
    kidnappersWin() {
        ...
        for (const P of this.players) {
            if (
                ...
                P.Appearance.InventoryGet("ItemArms")?.Extended?.Type ===
                    "BoxTie"
            ) {
                this.freePlayerInItemSlots(P, listOfUsedItemGroups);
        ...
        this.persistent_copy_of_kidnappers.forEach((K) =>
            this.freePlayerInItemSlots(K, listOfUsedItemGroups),
        );
        ...
        this.conn.SendMessage(
            "Emote",
            `*GAME: The club members were kidnapped one by one, unable to stop it. Eventually, ` +
    ...
    clubMembersWin() {
        ...
        for (const p of this.players) {
            if (
                p.MemberNumber !== this.stalker?.MemberNumber &&
        ...
        this.persistent_copy_of_kidnappers.forEach((K) =>
            this.tieUpCharacterAndRemove("club revenge", K),
        );
        ...
        this.conn.SendMessage(
            "Emote",
            `*GAME: ... and with this, the kidnappings stopped. The club members triumphed over ` +
    ...
    playerCheck(character: API_Character) {
        ...
        if (itemsCannotRemove.length > 0) {
            character.Tell(
                "Chat",
                `Warning: The game will conflict with following restraints you have on you:\n` +
        ...
        if (itemsToRemove.length !== 0) {
            character.Tell(
                "Chat",
                `Warning: Since you are currently tied up, please note that the game will remove the following ` +
        ...
        if (itemsCannotUse.length > 0) {
            character.Tell(
                "Chat",
                `Warning: The game uses following items, but you have them blocked or limited:\n` +
    ...
    private sendRoleMessage(character: API_Character): void {
        if (
            this.persistent_copy_of_kidnappers.size === 1 &&
            this.persistent_copy_of_kidnappers.has(character)
        ) {
            character.Tell("Whisper", solo_kidnapper_intro);
        } else if (this.persistent_copy_of_kidnappers.has(character)) {
            character.Tell(
                "Whisper",
                `GAME: You are one of the two kidnappers. You need to prevent everyone else ` +
                    `from finding out what you are doing at night. The kidnapper team consists of: ` +
                    `${Array.from(this.persistent_copy_of_kidnappers.values())
                        .map((A) => A.toString())
                        .join(", ")}. ` +
            ...
            character.Tell(
                "Whisper",
                `GAME: Please wait for the next game round to start in order to get a new role assignment.`,
            );
        }
    }
