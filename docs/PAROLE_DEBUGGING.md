...
for (const item of currentAppearance) {
    ...
    if (diag.isDiscrepancy) {
        console.log(
            `[DIAGNOSTIC] Group: ${item.Group} | Forward: ${diag.forward} | Inverse: ${diag.inverse}`,
        );
    }
}
```
...
2. Try: Implement retry logic:
    ...
    const stale = this.detectStaleCacheIndicators(
        appearance,
        previousAppearance,
    );
...
2. Add to `checkParoleViolation()`:
    ...
    for (const item of currentAppearance) {
        const diag = this.diagnoseClothingDetection(item.Group);
        console.log(
            `[DIAGNOSTIC] ${item.Name}: ${diag.forward} (forward) vs ${diag.inverse} (inverse)`,
        );
    }
    ```
